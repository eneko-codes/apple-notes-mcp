...
Do not modify, move, or delete an existing note. Test notes in the owner's real Notes library are live data: they are allowed only under the exception below, and when allowed must be clearly named `TESTING: ...` (in the first line, since Notes takes the title from it) and cleaned up when done.
